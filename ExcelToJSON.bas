Sub exportJSON()
  ...
  For j = 1 To numRows
    ...
    tempObj = tempObj + vbTab + "{" + tempRow + "}"   ' wrap row in curly braces and add a carriage return
    If j < numRows Then tempObj = tempObj + ","       ' skip the comma on last row
    tempObj = tempObj + vbCrLf + vbTab                ' add a carriage return
  Next ' Next j (row in spreadsheet)
    
    
  ' Wrap row data with curly braces and a 'collection name'
  tempObj = "{" + vbCrLf + vbTab _
  + Chr(34) + collectionName + Chr(34) + ": [" + vbCrLf + vbTab _
  + tempObj _
  + "]" + vbCrLf + _
  "}"
